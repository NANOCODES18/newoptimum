@include('layouts.price-head')


<div class="invest-pos">
    <img src="user/assets/images/earth.png" alt="" class="investment" />
    <img src="user/assets/images/bitcoin.png" alt="" />
  </div>
</div>


<!-- investment section -->
<figure>
<h1 style="z-index:5;">Real Estate Investment</h1>
<div class="pricing-container">
  <div class="card-pricing">
      <div class="card-title">
          <h2>Star</h2>
          <p><i class="fa fa-dollar"></i><Span>500</Span> -Minimum</p>
      </div>
      <div class="card-content">
          <ul>
              <li><i class="fa fa-dollar"></i><Span>49999</Span> -maximum</li>
              <li><i class="fa fa-check-circle"></i>7.5%  monthly Return</li>
              <li><i class="fa fa-check-circle"></i>  4 times</li>
              <li><i class="fa fa-check-circle"></i>100% ROI Each time</li>
              <li></i><i class="fa fa-dollar"></i>1500+ Total Return</li>
              <li><i class="fa fa-check-circle"></i>2 months duration</li>
              <li><i class="fa fa-check-circle"></i>24/7 Support</li>
          </ul>
          <button><a href="#">Get Started</a></button>
      </div>
  </div>
  <div class="card-pricing">
    <div class="card-title">
      <h2>Top</h2>
      <p><i class="fa fa-dollar"></i><Span>50000</Span> -Minimum</p>
  </div>
  <div class="card-content">
    <ul>
        <li><i class="fa fa-dollar"></i><Span>100000000</Span> -maximum</li>
        <li><i class="fa fa-check-circle"></i>9%  monthly Return</li>
        <li><i class="fa fa-check-circle"></i>  4 times</li>
        <li><i class="fa fa-check-circle"></i>100% ROI Each time</li>
        <li></i><i class="fa fa-dollar"></i>52,000+ Total Return</li>
        <li><i class="fa fa-check-circle"></i>5 months duration</li>
        <li><i class="fa fa-check-circle"></i>24/7 Support</li>
    </ul>
    <button><a href="#">Get Started</a></button>
</div>
  </div>
  
</div>
</div>
</figure>


@include('layouts.price-foot')
